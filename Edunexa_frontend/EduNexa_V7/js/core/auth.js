...
function chooseLogin(role,element){
    ...
    const staffTypes = document.getElementById("staffLoginTypes");
    if(staffTypes){
        staffTypes.classList.toggle("hidden", role !== "staff");
    }

    if(role === "staff" && !staffLoginRole){
        staffLoginRole = "faculty";
    }

}

function chooseStaffLogin(role, element){

    staffLoginRole = role;

    document
        .querySelectorAll("#staffLoginTypes .staff-type")
...
<<<<<<< HEAD:Edunexa Frontend/js/core/auth.js
        user.hodId =
            "HOD-" + Date.now();
...
>>>>>>> Main:Edunexa_frontend/EduNexa_V7/js/core/auth.js
...
function login(){
    ...
    const effectiveRole =
        loginRole === "staff" ? staffLoginRole : loginRole;
