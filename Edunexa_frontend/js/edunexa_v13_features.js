...
(function(){
  ...
  async function classDetailsV13(){
    if(currentUser?.role!=='student'||!api())return;try{const d=await api()('/student/class-details');const page=document.getElementById('student-timetable');if(!page)return;const info=page.querySelector('#eduClassInfo');if(info){info.innerHTML=`<div class="card-head"><div><h3>🏫 Class Information</h3></div></div><div class="grid3"><div class="item"><b>Class</b><p>${escV13(d.class_name||d.batch)}</p></div><div class="item"><b>Year / Batch</b><p>${escV13(d.class_batch||d.batch)}</p></div><div class="item"><b>Class Adviser</b><p>${escV13(d.class_adviser_name||'—')}</p></div><div class="item"><b>Mentor</b><p>${escV13(d.mentor_name||'—')}</p></div><div class="item"><b>HOD</b><p>${escV13(d.hod_name||'—')}</p></div><div class="item"><b>Department</b><p>${escV13(d.department||'—')}</p></div></div>`;}const t=document.getElementById('studentTimetable');if(t)t.innerHTML=`<div class="table-wrap"><table><thead><tr><th>Day</th><th>Period</th><th>Time</th><th>Subject</th><th>Subject Faculty</th><th>Room</th></tr></thead><tbody>${(d.timetable||[]).map(r=>`<tr><td>${escV13(r.day)}</td><td>${escV13(r.period)}</td><td>${escV13((r.start_time||'')+(r.end_time?' - '+r.end_time:''))}</td><td>${escV13(r.subject)}</td><td>${escV13(r.faculty_name)}</td><td>${escV13(r.room||'—')}</td></tr>`).join('')||'<tr><td colspan="6">No timetable entries.</td></tr>'}</tbody></table></div>`;}catch(e){console.warn('V13 class details',e);}}
