...
CREATE INDEX IF NOT EXISTS idx_assignments_adviser ON student_staff_assignments(class_adviser_id);
CREATE INDEX IF NOT EXISTS idx_assignments_mentor ON student_staff_assignments(mentor_id);
...
CREATE TABLE IF NOT EXISTS faculty_attendance(id INTEGER PRIMARY KEY AUTOINCREMENT,faculty_id INTEGER REFERENCES users(id) ON DELETE CASCADE,date TEXT NOT NULL,status TEXT NOT NULL,check_in_time TEXT,check_out_time TEXT,remarks TEXT,marked_by INTEGER REFERENCES users(id),created_at TEXT DEFAULT CURRENT_TIMESTAMP,UNIQUE(faculty_id,date));
